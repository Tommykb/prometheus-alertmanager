...
func New(conf *config.Signl4Config, t *template.Template, l log.Logger) (*Notifier, error) {
	client, err := commoncfg.NewClientFromConfig(*conf.HTTPConfig, "signl4", false, false)
	if err != nil {
		return nil, err
	}
	return &Notifier{
		...
		logger: l,
		client: client,
		// SIGNL4 HTT requests are assumed to respond with 2xx response codes on a successful
